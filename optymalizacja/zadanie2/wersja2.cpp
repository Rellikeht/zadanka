  ...
  for (size_t k = 0; k < n - 1; ++k) {
    ...
    for (size_t i = k; i < n; ++i) {
      ...
      for (size_t j = k; j < n; ++j) {
        double val = std::abs(row[colIndex[j]]);
        if (val > maxVal) {
          maxVal = val;
          iMax = i;
          jMax = j;
    ...
    if (iMax != k) {
      double *rowK = A + k * n;
      double *rowI = A + iMax * n;
      for (size_t j = 0; j < n; ++j)
        std::swap(rowK[j], rowI[j]);
      std::swap(b[k], b[iMax]);
    }
    ...
    double *rowK = A + k * n;
    double pivot = rowK[colIndex[k]];
    for (size_t i = k + 1; i < n; ++i) {
      double *rowI = A + i * n;
      double factor = rowI[colIndex[k]] / pivot;
      for (size_t j = k; j < n; ++j) {
        rowI[colIndex[j]] -= factor * rowK[colIndex[j]];
      }
      b[i] -= factor * b[k];
    }
  }

  if (A[(n - 1) * n + colIndex[n - 1]] == 0.0)
    throw std::runtime_error(
        "Matrix is singular (zero pivot encountered)."
    );

  std::vector<double> y_back(n);
  double *y = y_back.data();
  for (int i = int(n) - 1; i >= 0; --i) {
    double sum = b[i];
    double *row = A + i * n;
    for (size_t j = i + 1; j < n; ++j)
      sum -= row[colIndex[j]] * y[j];
    y[i] = sum / row[colIndex[i]];
  }
