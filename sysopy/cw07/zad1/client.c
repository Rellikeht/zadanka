...
int main() {
     ...
     * and write
     */
    mqd_t mq_client_descriptor = mq_open(
        queue_name,
        O_RDWR | O_CREAT,
        S_IRUSR | S_IWUSR,
        &attributes
     ...
     * communication
     */
    mqd_t mq_server_descriptor = mq_open(
        SERVER_QUEUE_NAME, O_RDWR, S_IRUSR | S_IWUSR, NULL
    );
     ...
     */
    message_t message_init = {.type = INIT, .identifier = -1};

    // copy queue name to message buffer, MIN is used to prevent
    // buffer overflow
    memcpy(
        message_init.text,
        queue_name,
        MIN(CLIENT_QUEUE_NAME_SIZE - 1, strlen(queue_name))
    );
    ...
    if (listener_pid < 0)
        ...
        while (!should_close) {
            ...
            case IDENTIFIER:
                printf(
                    "Received identifier from server: %d\n",
                    receive_message.identifier
                );
                ...
                write(
                    to_parent_pipe[1],
                    &receive_message.identifier,
                    sizeof(receive_message.identifier)
                );
                break;
            default:
                printf(
                    "Unknown message type in client queue: %d",
        ...
        while (!should_close) {
            ...
            /**
             *  save version of scanf - allocates memory with
             * string of given length thanks to %ms parameter
             * specifier, needs to be free after prevent buffer
             * overflow
             * */
            if (scanf("%ms", &buffer) == 1) {
                ...
                // copy message to message buffer, MIN is used
                // to prevent buffer overflow
                memcpy(
                    send_message.text,
                    buffer,
                    MIN(MESSAGE_BUFFER_SIZE - 1, strlen(buffer))
                );
         ...
         */
        if (identifier != -1) {
            /** Prepare notification for the server that client
             * is closing */
