...
int main(int argc, char *argv[]) {
    ...
    /* socket(domain, type, protocol) - creates a new socket and
       returns its descriptor
        - domain: AF_INET indicates that we are using IPv4
        - type: SOCK_STREAM indicates that we are using a
       connection-oriented stream protocol (TCP)
        - protocol: indicates the protocol to be used with the
       socket. A value of 0 indicates that the default protocol
       (TCP) will be used. */
    ...
    /* struct sockaddr_in
        short            sin_family;   - this is always set to
       AF_INET because sockaddr_in is always used with IPv4
        unsigned short   sin_port;     - port number
        struct in_addr   sin_addr;     - IP address on which the
       server will listen. INADDR_ANY means that the server will
       listen on all available interfaces of the running machine
        char             sin_zero[8];  - this is padding to make
       struct the same size as sockaddr */
    struct sockaddr_in server_addr;
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = inet_addr(IP_ADDRESS);
    /* htons() converts the unsigned short integer hostshort
     * from host byte order to big-endian network byte order
     * (commonly used in networking protocols) */
    server_addr.sin_port = htons(PORT);

    /* bind(socket_descriptor, address structure,
     * address_length) - assigns the address to the socket */
    if (bind(
    ...
    /* listen(socket_descriptor, backlog) - marks the socket as
       a passive socket, that is, as a socket that will be used
       to accept incoming connection requests using accept
        - backlog: the maximum length to which the queue of
       pending connections for socket_descriptor may grow */
    ...
    while (1) {
        ...
        // waits for one of a set of file descriptors to become
        ...
        for (int i = 1; i < MAX_CLIENTS; i++) {
            if (poll_fds[i].fd != -1 &&
                ...
                int bytes_read = read(
                    poll_fds[i].fd, buffer, sizeof(buffer) - 1
                );
                if (bytes_read <= 0) {
                    if (bytes_read == 0) {
                        printf(
                        ...
                        perror("Error while reading data from "
                               "client");
                    }
                    close(poll_fds[i].fd);
                    poll_fds[i].fd = -1;
