...
char *get_command(const char *input) {
    static char result[256]; // Zakładając, że maksymalna
                             // długość stringa to 256 znaków
    int i = 0;

    // Przechodzenie przez string do napotkania spacji lub końca
    // stringa
...
char *get_param_after_command(const char *input) {
    ...
    // Przechodzenie przez string w poszukiwaniu pierwszej i
    // drugiej spacji
    while (input[i] != '\0') {
        if (input[i] == ' ') {
            if (start == -1) {
    ...
    result[j - start - 1] =
        '\0'; // Dodanie null-terminatora na końcu
