...
void *handler(int client_socket) {
    ...
    int id_len = recv(
        client_socket, id, MAX_IDENTIFIER_LEN, 0
    ); // CHECK IF NOT &id
    ...
    while (g_keep_alive) {
        int received =
            recv(client_socket, buff, BUFFER_SIZE, 0); // &buff?
        if (received == -1)
            perror("recv");
        ...
        if (strncmp(buff, "LIST", 4) == 0) {
            ...
            time_t now = time(0);
            strftime(
                message, 50, "[%Y-%m-%d %H:%M:%S] ", localtime(&now)
            );
            strcat(message, id);
            strcat(message, ": ");
            ...
            for (int i = 0; i < MAX_CLIENTS; ++i) {
                if (clients[i].socket != 0 &&
                    strcmp(clients[i].id, recipient_id) == 0) {
                    send(
                        clients[i].socket,
                        full_message,
                        strlen(full_message),
...
void *check_alive_clients(void *a) {
    UNUSED(a);
    while (1) {
        sleep(3);
        pthread_mutex_lock(&clients_mutex);
...
int main(int argc, char *argv[]) {
    ...
    server_addr.sin_port =
        htons(atoi(argv[2])); // potentially its not htons,
                              // since atoi could return 32-bit
