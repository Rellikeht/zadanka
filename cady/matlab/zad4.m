...
function XX=bitmap_h(
    ...
    outdir = fullfile(pwd, 'img/bitmap_h_iters');
    if ~exist(outdir, 'dir')
        mkdir(outdir);
    end
    [~, imgbase, ~] = fileparts(filename);
    ...
    while (redo_error_test && (refinemenet_level < max_refinement_level))
            ...
            if (elements(i).active)
                ...
                switch channel
                    ...
                    case "blue"
                        cond = bb >= maxerror;
                    case "all"
                        cond = (rr >= maxerror) || (gg >= maxerror) || (bb >= maxerror);
                end
                if (cond)
                    redo_error_test = true;
                    break_element(i);
        ...
        if save_iterations
            ...
            for i=1:total_elements
                elem = elements(i);
                if (elem.active)
                    interpolate_elem(elem,color_edges_black);
                end
            end
            switch channel
                case "red"
            ...
            img = uint8(XX);
            fname = fullfile(outdir, sprintf('%s_iter%03d.png', imgbase, refinemenet_level));
            imwrite(img, fname);
            RR = CR;
            GG = CG;
            BB = CB;
    ...
    function v3=interpolate_rgb(v1,v2,element)
        ...
        vdl = vertexes(elem.dl);
        width = vertexes(elem.dr).x - vdl.x;
        hight = vertexes(elem.ul).y - vdl.y;
        vert1 = vertexes(v1);
        vert2 = vertexes(v2);
    ...
    function interpolate_elem(elem,color_edges_black)
        vdl = vertexes(elem.dl);
        vul = vertexes(elem.ul);
        vdr = vertexes(elem.dr);
        vur = vertexes(elem.ur);
        w = vertexes(elem.dr).x - vdl.x;
        h = vertexes(elem.ul).y - vdl.y;
        w = abs(w);
        h = abs(h);
        dlx = vdl.x;
        dly = vdl.y;

        % create grid of local coords (rows vary by xx, cols by yy)
        [lXX,lYY] = ndgrid(0:w, 0:h);   % XX and YY are (w+1)-by-(h+1)
        % avoid division by zero for 1-pixel wide elems
        if w==0, X = zeros(size(lXX)); else X = lXX ./ w; end
        if h==0, Y = zeros(size(lYY)); else Y = lYY ./ h; end
        F1 = (1 - X) .* (1 - Y);
        F2 = (1 - X) .* Y;
        F3 = X .* (1 - Y);
        F4 = X .* Y;
        ...
        switch channel
            case "red"
                Rmat = floor(vdl.r.*F1 + vul.r.*F2 + vdr.r.*F3 + vur.r.*F4);
                % write columns at once (faster than nested assignment)
                for col = 0:h
                    RR(dlx:dlx+w, dly+col) = Rmat(:, col+1);
                end
                % color edges black (vectorized)
                if color_edges_black
                    RR(dlx:dlx+w, dly) = 0;
                    RR(dlx:dlx+w, dly+h) = 0;
                    RR(dlx, dly:dly+h) = 0;
                    RR(dlx+w, dly:dly+h) = 0;
                end

            case "green"
                Gmat = floor(vdl.g.*F1 + vul.g.*F2 + vdr.g.*F3 + vur.g.*F4);
                % write columns at once (faster than nested assignment)
                for col = 0:h
                    GG(dlx:dlx+w, dly+col) = Gmat(:, col+1);
                end
                % color edges black (vectorized)
                if color_edges_black
                    GG(dlx:dlx+w, dly) = 0;
                    GG(dlx:dlx+w, dly+h) = 0;
                    GG(dlx, dly:dly+h) = 0;
                    GG(dlx+w, dly:dly+h) = 0;
                end

            case "blue"
                Bmat = floor(vdl.b.*F1 + vul.b.*F2 + vdr.b.*F3 + vur.b.*F4);
                % write columns at once (faster than nested assignment)
                for col = 0:h
                    BB(dlx:dlx+w, dly+col) = Bmat(:, col+1);
                end
                % color edges black (vectorized)
                if color_edges_black
                    BB(dlx:dlx+w, dly) = 0;
                    BB(dlx:dlx+w, dly+h) = 0;
                    BB(dlx, dly:dly+h) = 0;
                    BB(dlx+w, dly:dly+h) = 0;
                end
    ...
    function [error_r,error_g,error_b]=estimate_error(index)
        ...
        x1 = floor(elementWidth/2.0 - elementWidth / (sqrt(3.0) * 2.0));
        x2 = floor(elementWidth/2.0 + elementWidth / (sqrt(3.0) * 2.0));
        y1 = floor(elementHeigth/2.0 - elementHeigth / (sqrt(3.0) * 2.0));
        y2 = floor(elementHeigth/2.0 + elementHeigth / (sqrt(3.0) * 2.0));

        % helper to compute interpolated rgb at a local (xx,yy)
        function [rrr,ggg,bbb] = interp_local(xx,yy)
            xi = xx / elementWidth;
            yi = yy / elementHeigth;
            f1 = (1-xi)*(1-yi);
            f2 = (1-xi)*yi;
            ...
            switch channel
                case "red"
                    rrr = floor(vdl.r*f1 + vul.r*f2 + vdr.r*f3 + vur.r*f4);
                    ggg = 0;
                    bbb = 0;
                case "green"
                    rrr = 0;
                    ggg = floor(vdl.g*f1 + vul.g*f2 + vdr.g*f3 + vur.g*f4);
                    bbb = 0;
                case "blue"
                    rrr = 0;
                    ggg = 0;
                    bbb = floor(vdl.b*f1 + vul.b*f2 + vdr.b*f3 + vur.b*f4);
                case "all"
                    rrr = floor(vdl.r*f1 + vul.r*f2 + vdr.r*f3 + vur.r*f4);
                    ggg = floor(vdl.g*f1 + vul.g*f2 + vdr.g*f3 + vur.g*f4);
                    bbb = floor(vdl.b*f1 + vul.b*f2 + vdr.b*f3 + vur.b*f4);
        ...
        [r1,g1,b1] = interp_local(x1,y1);
        [r2,g2,b2] = interp_local(x1,y2);
        [r3,g3,b3] = interp_local(x2,y1);
        [r4,g4,b4] = interp_local(x2,y2);
